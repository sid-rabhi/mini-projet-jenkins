...
pipeline {
          ...
          steps {
            script {
              sh '''
                  ...
                  docker ps -a | grep -i ${IMAGE_NAME} | awk '{print $1}' | xargs -I {} docker rm -f {}
  ...
  post {
       success {
         slackSend (color: '#00FF00', message: "SID - SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL}) - PROD URL => http://${PROD_APP_ENDPOINT} , STAGING URL => http://${STG_APP_ENDPOINT}")
         }
      failure {
            slackSend (color: '#FF0000', message: "SID - FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
          }   
    }  
}
